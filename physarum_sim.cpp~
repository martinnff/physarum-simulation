	...
	for(int i =0; i<100000; i++){
		...
		for(int p = 0; p<fin.cols*2;p++){
			for(int k=0;k<fin.rows;k++){
			    ...
			    fin.at<cv::Vec3f>(k,p).val[0]=cmp2(255,fin.at<cv::Vec3f>(k,p).val[0]+green.at<float>(cv::Point(p,k))*0.05);
			    fin.at<cv::Vec3f>(k,p).val[2]=cmp2(255,fin.at<cv::Vec3f>(k,p).val[2]+green.at<float>(cv::Point(p,k))*0.5);
			    fin.at<cv::Vec3f>(k,p).val[4]=cmp2(255,fin.at<cv::Vec3f>(k,p).val[4]+green.at<float>(cv::Point(p,k))*1);
